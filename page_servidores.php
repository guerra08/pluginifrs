...
echo '<div id = "form_pagesistec">';
	echo '<p>Selecione os filtros:</p>';
		echo '<form class="sistecselectform" action="page_servidores.php" method="get">'."\n".
...
if (!empty($roleid)) {
    ...
    $sql = "SELECT * FROM (
      SELECT u.firstname, u.lastname, c.shortname, cc.timecompleted, cc.course, cc.userid
      ...
      ORDER BY c.shortname, u.firstname ) AS servridores 
      WHERE servridores.userid IN (
        SELECT gm.userid
        FROM {groups_members} gm, {groups} g
        WHERE g.name = 'Servidor do IFRS' AND g.id = gm.groupid
      )
    ";
	}
  /* IF IT'S NOT SELECTED, THE DEFAULT QUERY IS USED */
  else {
		$sql = "SELECT cc.userid, u.firstname, u.lastname, cc.timecompleted, g.id
			FROM {user} u, {course_completions} cc, {groups} g, {groups_members} gm
			WHERE cc.timecompleted between $startdate and $enddate
			and cc.course = '".$id."'
			and u.id = cc.userid and g.name = 'Servidor do IFRS' and g.id = gm.groupid and u.id = gm.userid
