...
echo '<div id = "form_pagesistec">';
	echo '<p>Selecione os filtros:</p>';
	echo '<form class="sistecselectform" action="page_sistec.php" method="get">'."\n";
...
if (!empty($roleid)) {
	...
	//Verifies if Get values are setted, and stores them in normal variables (easier to manipulate)
    ...
    //If the user decides to search all the courses, the following query is used

	    $sql = "SELECT u.firstname, u.lastname, c.shortname, cc.timecompleted, cc.course, cc.userid
    ...
    //Prints the Heading of the page, alog with the number of counted users retrieved from the query.

    echo '<h3>Concluintes (com e sem CPF): '.get_string('counteditems', '', $a).'</h3>'."\n";
	$cpfs = ""; // Decalring empty CPFS variable. Will later store all the CPFs from each user.

    if($searchAllCourses == 1){
    
    // If the option to search all courses is selected, the following code will be used.
     
      $countUsers = 0;
      $sortedUsers = array(); // Empty array for future sorting of users by name.
        ...
        foreach ($sortedUsers as $users) { // For each course that is read, it's values are added to the table.
          ...
          asort($users); // Users are sorted by alphabetical order
          foreach ($users as $user){ // For each user, their data (in this case, CPF) is retrieved from the database
            $countUsers ++; 
            $sql = "SELECT ud.data
	        		FROM {user_info_data} ud
	        		JOIN {user_info_field} uf ON uf.id = ud.fieldid
	        		WHERE ud.userid = :userid AND uf.shortname = :fieldname"; // This SQL query retrieves each user's CPF
    ...
    foreach ($users as $u) {
      $countUsers++;
  		...
  		$cpf = str_replace(",", "", $cpf);
  		$cpf = str_replace(" ", "", $cpf);
  		$enrollmentDate = (isset($_GET["dia"]) && $_GET["dia"] == 1) ? date('d/m/Y H:i:s', $u->timecompleted): "";
  		$data = array ($cpf, $u->firstname.' '.$u->lastname, $enrollmentDate);
      if(isset($validateCPF) && $validateCPF == 1){
        if($cpf == ''){
          unset($data);
          $countUsers--;
