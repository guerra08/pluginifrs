...
$baseurl =  $CFG->wwwroot.'/report/sistec/cursosvazios.php?id='.$course->id.'&amp;roleid='
...
//if (!empty($roleid)) {
    ...
    $table->define_headers(array('Prefixo','Nome do curso','Usuários'));
    ...
    $sql = "SELECT c.shortname, c.fullname, count( ra.id ) AS enrolled
            FROM {course} c
            JOIN {context} ct ON ( ct.instanceid = c.id )
            LEFT JOIN {role_assignments} ra ON (ra.contextid = ct.id)
            WHERE ct.contextlevel = 50 
            GROUP BY c.shortname, c.fullname
            HAVING (count( ra.id ) <= 1)
            ORDER BY enrolled
    "; // Query results in courses with 5 or less users enrolled
...
/* IF THE QUERY IS NOT SUCCESSFUL, THE ARRAY USERS IS LEFT BLANK, OTHERWISE, IT WILL STORE THE QUERY RESULTS */
    if (!$courses = $DB->get_records_sql($sql)) {
        $courses = array(); // tablelib will handle saying 'Nothing to display' for us.
    }
    $data = array();
    $a = new stdClass();
    $a->count = count($courses);
    $a->items = $role->name;

    foreach ($courses as $key => $curso) { // For each course, it's name and ammount of enrolled users are stored in an array.
        $data = array($curso->shortname, $curso->fullname, $curso->enrolled);
        $table->add_data($data); // Data is sent to table.
    }

    echo '<h3>Usuários não confirmados: </h3>'."\n";
