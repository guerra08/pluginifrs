...
/* IF THE QUERY IS NOT SUCCESSFUL, THE ARRAY USERS IS LEFT BLANK, OTHERWISE, IT WILL STORE THE QUERY RESULTS */
    ...
    foreach ($users as $key => $user) {
      if (filter_var($user->email, FILTER_VALIDATE_EMAIL)){
        $data = array($user->id, $user->firstname.' '.$user->lastname, $user->email);
        $table->add_data($data);
    ...
    echo "<form method = 'get' action = 'page_confirmar.php'>
            <input type='hidden' name='id' value='".$course->id."' />
            <input type='hidden' name='roleid' value='1' />
            <input type='hidden' name='confirmUsers' value='1' />
            <input type = 'submit' value='Confirmar Usuários' />
        </form>
    ";
    ...
    if(isset($_GET['confirmUsers']) && $_GET['confirmUsers'] == 1){
    foreach ($users as $key => $user) {
      if (filter_var($user->email, FILTER_VALIDATE_EMAIL)){
        $sql = "UPDATE {user} SET confirmed = 1 WHERE id = $user->id";
        $DB->execute($sql);
      }
    }
    }
